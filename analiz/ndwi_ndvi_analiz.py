...
OUTPUT_CSV = 'analiz/csv/ndwi_ndvi_analiz_sonuclari.csv'
...
def main():
    ...
    for date, data in grouped_files.items():
        ...
        try:
            ...
            # --- Su Tespiti: NDWI > 0 VE NDVI < 0 ---
            water_mask = (ndwi > 0) & (ndvi < 0) & (~nodata_mask)
            ...
            print(f"   Su Tespiti: NDWI > 0 VE NDVI < 0 koşullarını sağlayan alan = {total_area_km2:.4f} km²")

            # Sadece geçerli piksellerin ortalamasını al (tüm alan için)
            ortalama_ndvi = np.mean(ndvi[~nodata_mask])
            print(f"   NDVI Sonucu: Tüm alandaki ortalama bitki örtüsü sağlığı = {ortalama_ndvi:.4f}")
